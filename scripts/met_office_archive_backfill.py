...
def main():
    ...
    ap.add_argument("--cycles", default="0,12",
                    help="Comma-separated cycle hours. 06Z/18Z cap at ~66h (re-verified 2026-05-05) so we "
                         "stick to the long-range 00Z + 12Z cycles for the archive backfill.")
    ap.add_argument("--leads", default="1,3,6,12,24,36,48,72,96,120",
                    help="Comma-separated lead hours. Matches the GFS/ECMWF archive-backfill set so all "
                         "archive sources align on the same lead grid. Global 00Z/12Z publishes hourly to "
                         "54h then 3-hourly to 144h then 6-hourly to 168h, so the full 1..120 set is available.")
