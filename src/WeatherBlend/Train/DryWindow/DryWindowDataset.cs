...
/// <summary>
/// Chronological 70/15/15 split of day-level training rows. Parallel to
/// <see cref="PrecipDataset"/> but splits on <see cref="DryWindowTrainingRow.TargetDateUtc"/>
/// (day granularity) instead of valid-time.
/// </summary>
    ...
    IReadOnlyList<DryWindowTrainingRow> Test)
{
    ...
    public int TrainPositives => Train.Count(r => r.HasDryWindow);
    public int ValPositives   => Val.Count(r => r.HasDryWindow);
    public int TestPositives  => Test.Count(r => r.HasDryWindow);
